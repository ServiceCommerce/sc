...
class Os_model extends CI_Model {
    ...
    public function autoCompleteProduto($q){
        ...
        if($query->num_rows > 0){
            foreach ($query->result_array() as $row){
                $row_set[] = array('label'=>$row['nome'].' | Preço: R$ '.$row['precoVenda'].' | Estoque: '.$row['estoque'],'estoque'=>$row['estoque'],'id'=>$row['idProdutos'],'preco'=>$row['precoVenda']);
            }
    ...
    public function autoCompleteCliente($q){
        ...
        if($query->num_rows > 0){
            foreach ($query->result_array() as $row){
                $row_set[] = array('label'=>$row['nome'].' | Documento: '.$row['documento'],'id'=>$row['idClientes']);
            }
            echo json_encode($row_set);
        }
    }
    ...
    public function autoCompleteUsuario($q){
        ...
        if($query->num_rows > 0){
            foreach ($query->result_array() as $row){
                $row_set[] = array('label'=>$row['nome'].' | E-mail: '.$row['email'],'id'=>$row['idUsuarios']);
            }
            echo json_encode($row_set);
        }
    }
