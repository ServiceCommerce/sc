...
class Permissoes extends CI_Controller {
    ...
    function adicionar() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $permissoes = array(
                  ...
                  'aServicoS' => $this->input->post('aServicoS'),
                  'eServicoS' => $this->input->post('eServicoS'),
                  'dServicoS' => $this->input->post('dServicoS'),
                  'vServicoS' => $this->input->post('vServicoS'),
            ...
            if ($this->permissoes_model->add('permissoes', $data) == TRUE) {
                ...
                $this->session->set_flashdata('success', 'Perm-001');
                redirect(base_url() . 'index.php/permissoes/');
            } else {
    ...
    function editar() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->permissoes_model->edit('permissoes', $data, 'idPermissao', $this->input->post('idPermissao')) == TRUE) {
                $this->session->set_flashdata('success', 'Perm-002');
    ...
    function excluir(){
        ...
        if ($id == null){
            ...
            $this->session->set_flashdata('error','Perm-10002');
        ...
        $this->session->set_flashdata('success','Perm-003');
        redirect(base_url().'index.php/servicos/gerenciar/');
    }
    ...
    function desativar(){
        if($this->input->post('id')){
            ...
            if($this->permissoes_model->edit('permissoes', $query, 'idPermissao', $this->input->post('id'))){
                $this->session->set_flashdata('success', 'Perm-004');
                redirect(base_url('index.php/permissoes\/'));
            }else{
                $this->session->set_flashdata('error', 'Perm-10003');
                redirect(base_url('index.php/permissoes\/'));
            }


        }else{
            $this->session->set_flashdata('error', 'Perm-10004');
            redirect(base_url('index.php/permissoes\/'));
        }
    }
}
